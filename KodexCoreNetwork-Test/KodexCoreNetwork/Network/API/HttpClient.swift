...
class AlamoFireCall {
    ...
    let session:Session!
    init() {
        manager = ServerTrustManager(evaluators: ["sruser.fnnsolutions.com": DisabledTrustEvaluator(),"app.fakejson.com": DisabledTrustEvaluator(),"45.56.122.34":DisabledTrustEvaluator()])
        ...
        if(method == .get || method == .delete){
            ...
            AlamoFireCall.shared.session.request(api.url()
                ...
                case .success(let resultString):
                    success(resultString.parseJSONString as AnyObject?,headers)
                    break
                case .failure(let error):
                    failure(error as NSError,headers)
                }
